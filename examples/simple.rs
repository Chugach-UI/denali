...
fn main() {
    smol::block_on(async {
        ...
        let shm = registry.bind::<WlShm>(&mut conn).await.unwrap();
        let compositor = registry.bind::<WlCompositor>(&mut conn).await.unwrap();
        let xdg_wm_base = registry.bind::<XdgWmBase>(&mut conn).await.unwrap();
